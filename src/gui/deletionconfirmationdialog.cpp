    ...
    , m_ui(new Ui::DeletionConfirmationDialog)
{
    ...
    if (size == 1) {
        ...
        if (tmpName.length() > 80) {
            tmpName = name.mid(0, 80);
            tmpName.append("...");
        }
        m_ui->label->setText(tr("Are you sure you want to delete '%1' from the transfer list?",
            "Are you sure you want to delete 'ubuntu-linux-iso' from the transfer list?").arg(tmpName.toHtmlEscaped()));
    }
    else {
