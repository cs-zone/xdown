...
namespace
{
    QHash<BitTorrent::TorrentState, QColor> torrentStateColorsFromUITheme()
    {
        ...
        const TorrentStateColorDescriptor colorDescriptors[] =
        {
            ...
            {BitTorrent::TorrentState::XDown_Waiting, QLatin1String("TransferList.Waiting")},
            {BitTorrent::TorrentState::XDown_Downloading,QLatin1String("TransferList.Downloading")},
            {BitTorrent::TorrentState::XDown_Completed,QLatin1String("TransferList.PausedUploading")},
            {BitTorrent::TorrentState::XDown_Paused,QLatin1String("TransferList.PausedDownloading")},
            {BitTorrent::TorrentState::XDown_Error,QLatin1String("TransferList.Error")},
            {BitTorrent::TorrentState::XDown_Removing,QLatin1String("TransferList.Moving")},
            {BitTorrent::TorrentState::XDown_Merging,QLatin1String("TransferList.Downloading")},
            {BitTorrent::TorrentState::XDown_Converting,QLatin1String("TransferList.Downloading")},
...
TransferListModel::TransferListModel(QObject *parent)
    ...
    , m_statusStrings
    {
          ...
          {BitTorrent::TorrentState::XDown_Waiting,              tr("Waiting", "Task is queued")}, 
          {BitTorrent::TorrentState::XDown_Downloading,          tr("Downloading")},
          {BitTorrent::TorrentState::XDown_Completed,            tr("Completed")}, 
          {BitTorrent::TorrentState::XDown_Paused,               tr("Paused")},
          {BitTorrent::TorrentState::XDown_Error,                tr("Errored", "Task has an error")},
          {BitTorrent::TorrentState::XDown_Removing,             tr("Moving", "Task local data are being moved/relocated")},
          {BitTorrent::TorrentState::XDown_Merging,              tr("Merging", "Multi file Mergeing")},
          {BitTorrent::TorrentState::XDown_Converting,           tr("Convert", "File Converting format")}
      }
    , m_stateThemeColors {torrentStateColorsFromUITheme()}
{
    ...
    qDebug() << "init TransferListModel Before addXDown ";
    
    for (TorrentHandle *const torrent : asConst(Session::instance()->xdowns())) {
        addXDown(torrent);
    }
    ...
    connect(Session::instance(), &Session::xdownAdded, this, &TransferListModel::addXDown);
    connect(Session::instance(), &Session::xdownAboutToBeRemoved, this, &TransferListModel::handleXDownAboutToBeRemoved);
    connect(Session::instance(), &Session::xdownFinished, this, &TransferListModel::handleXDownStatusUpdated);
    connect(Session::instance(), &Session::xdownResumed, this, &TransferListModel::handleXDownStatusUpdated);
    connect(Session::instance(), &Session::xdownPaused, this, &TransferListModel::handleXDownStatusUpdated);
    connect(Session::instance(), &Session::xdownUpdated, this, &TransferListModel::handleXDownStatusUpdated);
}
...
QString TransferListModel::displayValue(const BitTorrent::TorrentHandle *torrent, const int column) const
{
    ...
    const auto amountString = [hideValues](const BitTorrent::TorrentHandle *torrent, const qint64 value, const qint64 total) -> QString
    {
    ...
    const auto progressString = [](const BitTorrent::TorrentHandle *torrent, qreal progress) -> QString
    {
        if (torrent->getHandleType() == BitTorrent::TaskHandleType::XDown_Handle) {
            if (torrent->wantedSize() > 0 && torrent->wantedSize() == torrent->completedSize()) {
                return QString::fromLatin1("100%");
            }
        }
        progress *= 100;
    ...
    const auto statusString = [this](const BitTorrent::TorrentState state,
    ...
    case TR_PROGRESS:
        return progressString(torrent, torrent->progress());
    case TR_STATUS:
        return statusString(torrent->state(), torrent->error());
    case TR_SEEDS:
        return amountString(torrent, torrent->seedsCount(), torrent->totalSeedsCount());
    case TR_PEERS:
        return amountString(torrent, torrent->leechsCount(), torrent->totalLeechersCount());
    case TR_DLSPEED:
...
void TransferListModel::addXDown(BitTorrent::TorrentHandle *const xdownItem)
{
    Q_ASSERT(!m_torrentMap.contains(xdownItem));

    const int row = m_torrentList.size();

    beginInsertRows({}, row, row);
    m_torrentList << xdownItem;
    m_torrentMap[xdownItem] = row;
    endInsertRows();
}
...
void TransferListModel::handleXDownAboutToBeRemoved(BitTorrent::TorrentHandle *const xdownItem)
{
    const int row = m_torrentMap.value(xdownItem, -1);
    Q_ASSERT(row >= 0);

    beginRemoveRows({}, row, row);
    m_torrentList.removeAt(row);
    m_torrentMap.remove(xdownItem);
    for (int &value : m_torrentMap) {
        if (value > row)
            --value;
    }
...
void TransferListModel::handleXDownStatusUpdated(BitTorrent::TorrentHandle *const xdownItem)
{
    const int row = m_torrentMap.value(xdownItem, -1);
    Q_ASSERT(row >= 0);
...
QIcon getIconByState(const BitTorrent::TorrentState state)
{
    ...
    case BitTorrent::TorrentState::Error:
        ...
        case BitTorrent::TorrentState::XDown_Waiting:
            return getQueuedIcon();
        case BitTorrent::TorrentState::XDown_Downloading:
            return getDownloadingIcon();
        case BitTorrent::TorrentState::XDown_Completed:
            return getCompletedIcon();
        case BitTorrent::TorrentState::XDown_Paused:
            return getPausedIcon();
        case BitTorrent::TorrentState::XDown_Error:
            return getErrorIcon();
        case BitTorrent::TorrentState::XDown_Removing:
            return getCheckingIcon();
        case BitTorrent::TorrentState::XDown_Merging:
            return getCompletedIcon();
        case BitTorrent::TorrentState::XDown_Converting:
            return getCompletedIcon();
...
QColor getDefaultColorByState(const BitTorrent::TorrentState state)
{
    ...
    case BitTorrent::TorrentState::XDown_Waiting:
        // Queue {0, 128, 128}; // Teal
        if (!dark)
            return { 12, 132, 160 }; // Black
        else
            return { 204, 204, 204 }; // Gray 80
    case BitTorrent::TorrentState::XDown_Downloading:
        if (!dark)
