...
void PluginSelectDialog::on_actionUninstall_triggered()
{
    ...
    if (error)
        QMessageBox::warning(this, tr("Uninstall warning"), tr("Some plugins could not be uninstalled because they are included in XDown. Only the ones you added yourself can be uninstalled.\nThose plugins were disabled."));
    else
...
void PluginSelectDialog::askForLocalPlugin()
{
    const QStringList pathsList = QFileDialog::getOpenFileNames(
                ...
                tr("XDown search plugin") + QLatin1String(" (*.py)")
                );
