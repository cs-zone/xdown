...
MainWindow::MainWindow(QWidget *parent, bool bIsHideWindow)
    : QMainWindow(parent)
    , m_ui(new Ui::MainWindow)
    , m_posInitialized(false)
    , m_forceExit(false)
    , m_unlockDlgShowing(false)
#ifdef __Q_BITTORRENT_UPDATE__
#if defined(Q_OS_WIN) || defined(Q_OS_MACOS)
    ...
    , m_hasPython(false)
{
    ...
    // Setting icons
#ifndef Q_OS_MACOS
    const QIcon appLogo(UIThemeManager::instance()->getIcon(QLatin1String("xdown32")));
    ...
    m_ui->actionDonateMoney->setVisible(false);
	...
	QString strLockTitle = tr("Lock");
    strLockTitle.append(" XDown");

    m_ui->actionLock->setText(strLockTitle);
    ...
    connect(BitTorrent::Session::instance(), &BitTorrent::Session::xdownNew, this, &MainWindow::xdownNew);

    connect(BitTorrent::Session::instance(), &BitTorrent::Session::OnXDownAddTask, this, &MainWindow::downloadFromURLList);

    connect(BitTorrent::Session::instance(), &BitTorrent::Session::xdownStartToMinimizeWindow,
    ...
    pLeftBoxLayout->addWidget(m_transferListFiltersWidget);


    QWidget *qLeftWidget = new QWidget;
    qLeftWidget->setLayout(pLeftBoxLayout);


    hSplitter->addWidget(m_transferListWidget);
    hSplitter->addWidget(m_propertiesWidget);
    m_splitter->addWidget(qLeftWidget);
    ...
    if (bIsHideWindow) {
        ...
        if (!pref->minimizeToTrayNotified()) {
            showNotificationBaloon(tr("XDown is minimized to tray"), tr("This behavior can be changed in the settings. You won't be reminded again."));
            pref->setMinimizeToTrayNotified(true);
        }
    } 
    else if (m_systrayIcon) {
    ...
    if (!pref->neverCheckFileAssoc() && (!Preferences::isTorrentFileAssocSet() || !Preferences::isMagnetLinkAssocSet()))
    {
        if (QMessageBox::question(this, tr("Torrent file association"),
                                  tr("XDown is not the default application to open torrent files or Magnet links.\nDo you want to associate XDown to torrent files and Magnet links?"),
...
#ifdef Q_OS_WIN
    // 
    qlonglong appHwnd = this->winId();
    BitTorrent::Session::instance()->SetMainAppHwnd(appHwnd);
#endif

    {
        // BitTorrent::Session::instance()
        connect(BitTorrent::Session::instance(), &BitTorrent::Session::startOnShowOptionDlg,
            this, &MainWindow::handleOnShowOptionDlg);

        connect(BitTorrent::Session::instance(), &BitTorrent::Session::startOnCreateTask,
            this, &MainWindow::handleOnCreateTask);


        connect(BitTorrent::Session::instance(), &BitTorrent::Session::startOnShowUpdateLinkDlg,
            this, &MainWindow::handleOnShowUpdateLinkDlg);
        

        connect(BitTorrent::Session::instance(),
            &BitTorrent::Session::OnEnableMenuAndButton,
            this, &MainWindow::handleEnableMenuAndButton);
    }
...
void MainWindow::handleEnableMenuAndButton(bool bIsEnable)
{
    m_bEnableCtrl = bIsEnable;
    onEnableMenuAndButton(bIsEnable);
}

void MainWindow::onEnableMenuAndButton(bool bIsEnable)
{
    m_ui->actionOpen->setEnabled(bIsEnable);
    m_ui->actionDownloadFromURL->setEnabled(bIsEnable);
    //m_ui->actionSetUploadLimit->setEnabled(bIsEnable);
    //m_ui->actionSetDownloadLimit->setIcon(UIThemeManager::instance()->getIcon("kt-set-max-download-speed"));
    //m_ui->actionSetGlobalUploadLimit->setIcon(UIThemeManager::instance()->getIcon("kt-set-max-upload-speed"));
    //m_ui->actionSetGlobalDownloadLimit->setIcon(UIThemeManager::instance()->getIcon("kt-set-max-download-speed"));
    m_ui->actionCreateTorrent->setEnabled(bIsEnable);
    //m_ui->actionAbout->setEnabled(bIsEnable);
    //m_ui->actionStatistics->setIcon(UIThemeManager::instance()->getIcon("view-statistics"));
    //m_ui->actionTopQueuePos->setIcon(UIThemeManager::instance()->getIcon("go-top"));
    //m_ui->actionIncreaseQueuePos->setIcon(UIThemeManager::instance()->getIcon("go-up"));
    //m_ui->actionDecreaseQueuePos->setIcon(UIThemeManager::instance()->getIcon("go-down"));
    //m_ui->actionBottomQueuePos->setIcon(UIThemeManager::instance()->getIcon("go-bottom"));
    m_ui->actionDelete->setEnabled(bIsEnable);
    //m_ui->actionDocumentation->setIcon(UIThemeManager::instance()->getIcon("help-contents"));
    //m_ui->actionDonateMoney->setIcon(UIThemeManager::instance()->getIcon("wallet-open"));
    //m_ui->actionDonateMoney->setVisible(false);
    //m_ui->actionExit->setIcon(UIThemeManager::instance()->getIcon("application-exit"));
    m_ui->actionLock->setEnabled(bIsEnable);
    //m_ui->actionOptions->setEnabled(bIsEnable);
    m_ui->actionPause->setEnabled(bIsEnable);
    m_ui->actionPauseAll->setEnabled(bIsEnable);
    m_ui->actionStart->setEnabled(bIsEnable);
    m_ui->actionStartAll->setEnabled(bIsEnable);
    //m_ui->menuAutoShutdownOnDownloadsCompletion->setIcon(UIThemeManager::instance()->getIcon("application-exit"));
    //m_ui->actionManageCookies->setIcon(UIThemeManager::instance()->getIcon("preferences-web-browser-cookies"));

    m_ui->actionUseAlternativeSpeedLimits->setEnabled(bIsEnable);
    m_ui->actionSetGlobalDownloadLimit->setEnabled(bIsEnable);
    m_ui->actionSetGlobalUploadLimit->setEnabled(bIsEnable);
...
{
    QDesktopServices::openUrl(QUrl("https://xdown.org/"));
}
...
void MainWindow::xdownNew(BitTorrent::TorrentHandle *const xdown) const
{
    if (isTorrentAddedNotificationsEnabled()) {
        if (xdown != nullptr) {
            QString strUrl = xdown->url();
            if (strUrl.length() > 0) {
                if (strUrl.startsWith("http")) {
                    showNotificationBaloon(tr("http added"), tr("'%1' was added.", "e.g: xxx.avi was added.").arg(xdown->name()));
                }
                else {
                    showNotificationBaloon(tr("ftp added"), tr("'%1' was added.", "e.g: xxx.avi was added.").arg(xdown->name()));
                }
            }
        }
    }
...
void MainWindow::notifyOfUpdate(const QString &)
{
    ...
    Logger::instance()->addMessage(tr("XDown was just updated and needs to be restarted for the changes to be effective.")
                                   , Log::CRITICAL);
...
void MainWindow::toggleVisibility(const QSystemTrayIcon::ActivationReason reason)
{
    ...
    case QSystemTrayIcon::Trigger:
        if (isHidden())
        {
            ...
            BitTorrent::Session::instance()->setXDownSysTrayIcon(0);
...
void MainWindow::closeEvent(QCloseEvent *e)
{
	...
	{
        BitTorrent::Session::instance()->setXDownSysTrayIcon(1);
        e->ignore();
        QTimer::singleShot(0, this, &QWidget::hide);
        if (!pref->closeToTrayNotified())
        {
            showNotificationBaloon(tr("XDown is closed to tray"), tr("This behavior can be changed in the settings. You won't be reminded again."));
    ...
    if (pref->confirmOnExit() && BitTorrent::Session::instance()->hasActiveTorrents())
    {
        if (e->spontaneous() || m_forceExit)
        {
            if (!isVisible())
                show();
            QMessageBox confirmBox(QMessageBox::Question, tr("Exiting XDown"),
                                   // Split it because the last sentence is used in the Web UI
                                   tr("Some files are currently transferring.") + '\n' + tr("Are you sure you want to quit XDown?"),
                                   QMessageBox::NoButton, this);
...
void MainWindow::reloadSessionStats()
{
    ...
    qlonglong curDownloadSpeed = BitTorrent::Session::instance()->getTotalDownloadSpeed();
#ifdef Q_OS_MACOS
    ...
    if (m_systrayIcon) {
        const QString toolTip = QString::fromLatin1("%1\n%2").arg(
            tr("DL speed: %1", "e.g: Download speed: 10 KiB/s").arg(Utils::Misc::friendlyUnit(curDownloadSpeed, true))
    ...
    if (m_displaySpeedInTitle) {
        setWindowTitle(tr("[Down: %1, Up: %2] XDown ", "D = Download; U = Upload; XDown")
            .arg(Utils::Misc::friendlyUnit(curDownloadSpeed, true)
...
{
    qDebug() << "====Entry downloadFromURLList====";
    const bool useTorrentAdditionDialog = AddNewTorrentDialog::isEnabled();
    for (const QString &url : urlList) {
        ...
        if (!tmpUrl.endsWith(".torrent")
            ...
            QHash<QString, QString>::const_iterator iter = urlToFileNameMap.find(strUrl.toLower().trimmed());
            QString strInputFileName = "";
            if (iter != urlToFileNameMap.end()) {
                strInputFileName = iter.value();
                QString strReplaceFileName = Utils::Gui::replaceBadFileName(strInputFileName);
                if (strReplaceFileName.length() > 0) {
                    strInputFileName = strReplaceFileName;
                }
            }

            BitTorrent::Session::instance()->addXDown(inputUrl, headerMap, optionMap, strInputFileName);
        }
        else {
            if (useTorrentAdditionDialog)
...
{
    OnShowOptionDlg(false,0);
}


void MainWindow::handleOnShowUpdateLinkDlg(const QString &strUrl)
{
    if (m_updateLinkDialog)
    {
        m_updateLinkDialog->activateWindow();
        m_updateLinkDialog->onInitCtrl();
    }
    else
    {
        m_updateLinkDialog = new XUpdateLinkDialog(this);
        connect(m_updateLinkDialog, &XUpdateLinkDialog::updateToOK, this, &MainWindow::handleUpdateLinkToOK);
        connect(m_updateLinkDialog, &XUpdateLinkDialog::updateToExit, this, &MainWindow::handleUpdateLinkToExit);
    }
    m_updateLinkDialog->setModal(true);
    m_updateLinkDialog->setPrvLinkValue(strUrl);
    m_updateLinkDialog->show();
}

void MainWindow::handleUpdateLinkToOK(const QString &strVal, const QString &strUpdateLinkVal)
{
    BitTorrent::Session *session = BitTorrent::Session::instance();
    // TEST
    session->updateXDown_impl(strVal, strUpdateLinkVal);
}

void MainWindow::handleUpdateLinkToExit()
{
}

void MainWindow::handleOnShowOptionDlg(bool bIsTop, int iActiveTab)
{
    OnShowOptionDlg(bIsTop, iActiveTab);
}

bool MainWindow::handleOnCreateTask(const QString& strJson)
{
    return actionOnCreateTask(strJson);
}

void MainWindow::OnEmptyTagXDownMsg()
{
    m_tagDownMsg.iMsgId = 0;
    m_tagDownMsg.iProcessId = 0;
    m_tagDownMsg.iLinkType = 0;
    m_tagDownMsg.reqHeaderMap.clear();
    m_tagDownMsg.reqLinkList.clear();
    m_tagDownMsg.reqMagnetLink = "";
}


// 1 http 普通下载, 支持批量
// 2 磁链下载, linkTxt 只会取第一个链接解析
bool MainWindow::actionOnCreateTask(const QString& strJson)
{
    OnEmptyTagXDownMsg();
    ...
    // 匹配json
    bool bMatchJson = false;
    QJsonParseError error;
    QJsonDocument cfgDoc = QJsonDocument::fromJson(strJson.toUtf8(), &error);
    if (!cfgDoc.isNull() && (error.error == QJsonParseError::NoError)) {
        QMap<QString, QVariant> result = cfgDoc.toVariant().toMap();
        m_tagDownMsg.iMsgId = result["id"].toLongLong();
        m_tagDownMsg.iProcessId = result["processId"].toLongLong();

        int iLinkType = result["linkType"].toInt();
        m_tagDownMsg.iLinkType = iLinkType;
        if(iLinkType == 1) {
            QMap<QString, QVariant> xDownHeader = result["httpHeaders"].toMap();
            QList<QString> xDownHeadList = xDownHeader.keys();
            for(int idx = 0; idx < xDownHeadList.size(); idx++) {
                QString strHeaderKey = xDownHeadList[idx];
                QString strHeaderValue = xDownHeader[strHeaderKey].toString();
                if (strHeaderKey.trimmed().length() > 0 && strHeaderValue.trimmed().length() > 0) {
                    m_tagDownMsg.reqHeaderMap.insert(strHeaderKey.trimmed(), strHeaderValue.trimmed());
                }
            }
            QVariant reqList = result["linkList"];
            if (reqList.isNull()) {
                return bMatchJson;
            }
            QVariantList xDownList = reqList.toList();
            for (int idx = 0; idx < xDownList.size(); idx++) {
                QMap<QString, QVariant> xDownItem = xDownList.at(idx).toMap();
                if (xDownItem.size() > 0) {
                    QTagLinkItem curItem;
                    curItem.fileName = xDownItem["fileName"].toString();
                    curItem.fileSize = xDownItem["fileSize"].toString();
                    curItem.linkTxt = xDownItem["linkTxt"].toString();
                    if (!session->CheckIsIgnoreDownload(curItem.linkTxt, curItem.fileName, curItem.fileSize.toLongLong())) {
                        // check is skip 
                        m_tagDownMsg.reqLinkList.push_back(curItem);
                    }
                }
            }
            if (m_tagDownMsg.reqLinkList.size() > 0) {
                bMatchJson = true;
            }
        }
        else if (iLinkType == 2) {
            m_tagDownMsg.reqMagnetLink = result["linkMagnet"].toString();
            bMatchJson = true;
        }
    }
    ...
    if (!bMatchJson) {
        // 回复失败
        if (m_tagDownMsg.iLinkType > 0) {
            session->sendFailedMessageToHosts(m_tagDownMsg.iMsgId, m_tagDownMsg.iProcessId);
        }
        return bMatchJson;
    }
...
void MainWindow::OnShowOptionDlg(bool bIsTop,int iActiveTab)
{
    if (m_options) {
    ...
    Qt::WindowFlags curFlags = m_options->windowFlags();
    if (bIsTop) {
        // 设置窗体置顶
        curFlags |= Qt::WindowStaysOnTopHint;
    }
    else {
        // 取消窗体置顶
        curFlags &= ~Qt::WindowStaysOnTopHint;
    ...
    if (iActiveTab > 0) {
        m_options->SetActiveTabToHttp();
    }
    m_options->setModal(true);
    m_options->setWindowFlags(curFlags);
    m_options->show();
}
...
bool MainWindow::CreateDownloadFromUrl()
{
    ...
    if (!m_downloadFromURLDialog) {
        ...
        m_downloadFromURLDialog = new DownloadFromURLDialog(this, m_tagDownMsg);
        connect(m_downloadFromURLDialog.data(), &DownloadFromURLDialog::urlsReadyToBeDownloaded, this, &MainWindow::downloadFromURLList);
        m_downloadFromURLDialog->setModal(true);
        m_downloadFromURLDialog->setWindowFlags(m_downloadFromURLDialog->windowFlags() | Qt::WindowStaysOnTopHint);
        m_downloadFromURLDialog->show();
    }
    else {
        BitTorrent::Session *const session = BitTorrent::Session::instance();
        session->sendFailedMessageToHosts(m_tagDownMsg.iMsgId, m_tagDownMsg.iProcessId);
...
{
    QDesktopServices::openUrl(QUrl("https://xdown.org/donate"));
}
...
QIcon MainWindow::getSystrayIcon() const
{
    ...
    {
    case TrayIcon::NORMAL:
        return UIThemeManager::instance()->getIcon(QLatin1String("xdown32"));
    case TrayIcon::MONO_DARK:
        return UIThemeManager::instance()->getIcon(QLatin1String("xdown32"));
    case TrayIcon::MONO_LIGHT:
        return UIThemeManager::instance()->getIcon(QLatin1String("xdown32"));
