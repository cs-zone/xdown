...
void PropertiesWidget::switchTorrentInfoCtrls(bool bValue)
{
    m_ui->labelDlLimit->setVisible(bValue);
    m_ui->labelDlLimitVal->setVisible(bValue);

    m_ui->labelUpLimit->setVisible(bValue);
    m_ui->labelUpLimitVal->setVisible(bValue);
    m_ui->labelWasted->setVisible(bValue);
    m_ui->labelWastedVal->setVisible(bValue);
    m_ui->labelRatio->setVisible(bValue);
    m_ui->labelShareRatioVal->setVisible(bValue);
    m_ui->labelReannounceIn->setVisible(bValue);
    m_ui->labelReannounceInVal->setVisible(bValue);
    m_ui->labelLastSeenComplete->setVisible(bValue);
    m_ui->labelLastSeenCompleteVal->setVisible(bValue);

    m_ui->labelHash->setVisible(bValue);
    m_ui->labelHashVal->setVisible(bValue);

    m_ui->labelDlSpeed->setVisible(bValue);
    m_ui->labelDlSpeedVal->setVisible(bValue);
    m_ui->labelUpSpeed->setVisible(bValue);
    m_ui->labelUpSpeedVal->setVisible(bValue);
    m_ui->labelPeers->setVisible(bValue);
    m_ui->labelPeersVal->setVisible(bValue);

    //m_ui->labelComment->setVisible(bValue);
    //m_ui->labelCommentVal->setVisible(bValue);
}
// 显示下载任务详细信息
void PropertiesWidget::loadTorrentInfos(BitTorrent::TorrentHandle *const torrent)
{
    ...
    bool bIsHttpValue = (m_torrent->getHandleType() == BitTorrent::TaskHandleType::XDown_Handle);

    if (bIsHttpValue) {
        m_ui->labelException->setVisible(true);
        m_ui->labelExceptionVal->setVisible(true);
        QString errorMessage = m_torrent->getErrorMessage();
        if (m_torrent->hasError() && errorMessage.length() > 0) {
            m_ui->labelExceptionVal->setText(errorMessage);
        }
        else {
            m_ui->labelExceptionVal->setText("");
        }
    }
    else {
        m_ui->labelException->setVisible(false);
        m_ui->labelExceptionVal->setVisible(false);
    }
    ...
    if (bIsHttpValue) {
        switchTorrentInfoCtrls(false);
    }
    else {
        switchTorrentInfoCtrls(true);
    }
...
void PropertiesWidget::saveSettings()
{
    ...
    if (m_state == VISIBLE)
        ...
        sizes = m_slideSizes;
    qDebug("== saveSettings Sizes: %d", sizes.size());
    if (sizes.size() == 2)
...
void PropertiesWidget::askWebSeed()
{
    ...
    {
        QMessageBox::warning(this, "XDown",
...
void PropertiesWidget::editWebSeed()
{
    ...
    {
        QMessageBox::warning(this, tr("XDown"),
                             tr("This URL seed is already in the list."),
                             QMessageBox::Ok);
        return;
    }
