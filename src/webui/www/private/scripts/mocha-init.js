...
const initializeWindows = function() {
    ...
    addClickEvent('about', function(e) {
        ...
        new MochaUI.Window({
            id: id,
            title: 'QBT_TR(About XDown)QBT_TR[CONTEXT=AboutDialog]',
            loadMethod: 'xhr',
    ...
    addClickEvent('shutdown', function(e) {
        new Event(e).stop();
        if (confirm('QBT_TR(Are you sure you want to quit XDown?)QBT_TR[CONTEXT=MainWindow]')) {
            new Request({
                url: 'api/v2/app/shutdown',
                onSuccess: function() {
                    document.write('<!doctype html><html lang="${LANG}"><head> <meta charset="UTF-8"> <title>QBT_TR(XDown has been shutdown)QBT_TR[CONTEXT=HttpServer]</title></head><body> <h1 style="text-align: center;">QBT_TR(XDown has been shutdown)QBT_TR[CONTEXT=HttpServer]</h1></body></html>');
                    document.close();
                    stop();
                }
            }).send();
