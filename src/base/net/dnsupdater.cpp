...
void DNSUpdater::checkPublicIP()
{
    ...
    LogMsg("[DNSUpdater] Start Check Public IP Address ");
...
void DNSUpdater::ipRequestFinished(const DownloadResult &result)
{
    if (result.status != DownloadStatus::Success)
    {
        ...
        LogMsg("[DNSUpdater] Check Public IP Address Failed, Check Proxy settings");
        return;
    }
    ...
    if (ipRegexMatch.hasMatch())
    {
        ...
        if (!newIp.isNull())
        {
            if (m_lastIP != newIp)
            {
                ...
                m_lastIP = newIp;
                strLogMsg = QString("[DNSUpdater] Check Public IP Address %1 and report the change to DynDNS").arg(ipStr);
                updateDNSService();
            }
            else 
			{
                strLogMsg = QString("[DNSUpdater] Check Public IP Address %1").arg(ipStr);
            }
        }
        else 
		{
            strLogMsg = QString("Failed to construct a QHostAddress from the IP string");
        }
    }
    else
	{
        strLogMsg = QString("[DNSUpdater] Regular expression failed to capture the IP address");
    }
    LogMsg(strLogMsg);
}
...
void DNSUpdater::updateDNSService()
{
    ...
    LogMsg("[DNSUpdater] report the change to DynDNS");
...
void DNSUpdater::ipUpdateFinished(const DownloadResult &result)
{
    QString strLogMsg;
    if (result.status == DownloadStatus::Success) {
        strLogMsg = "[DNSUpdater] DynDNS response success";
        LogMsg("[DNSUpdater] DynDNS response success");
        processIPUpdateReply(result.data);
    }
    else {
        strLogMsg = QString("[DNSUpdater] DynDNS IP update failed %1").arg(result.errorString);
...
void DNSUpdater::processIPUpdateReply(const QString &reply)
{
	...
	logger->addMessage(tr("Your dynamic DNS result %1").arg(reply));
    ...
    if (code == "badagent")
    {
        logger->addMessage(tr("Dynamic DNS error: XDown was blacklisted by the service, please report a bug at http://bugs.qbittorrent.org."),
                           Log::CRITICAL);
        m_state = FATAL;
        return;
    }
