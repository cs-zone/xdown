...
{
    // Ä¬ÈÏÐÞ¸ÄÎ´true
    return value("Preferences/General/PreventFromSuspendWhenDownloading", true).toBool();
}
...
bool Preferences::WinStartup() const
{
    ...
    return settings.contains("XDown");
}
...
void Preferences::setWinStartup(const bool b)
{
    ...
    if (b)
    {
        ...
        settings.setValue("XDown", binPath);
    }
    else
    {
        settings.remove("XDown");
    }
}
#endif // Q_OS_WIN
...
bool Preferences::isTorrentFileAssocSet()
{
    ...
    if (settings.value(".torrent/Default").toString() != "XDown")
    {
        qDebug(".torrent != XDown");
        return false;
    }
...
void Preferences::setTorrentFileAssoc(const bool set)
{
    ...
    if (set)
    {
        ...
        if (!oldProgId.isEmpty() && (oldProgId != "XDown"))
            settings.setValue(".torrent/OpenWithProgids/" + oldProgId, "");
        settings.setValue(".torrent/Default", "XDown");
    }
