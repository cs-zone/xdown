    ...
    , m_commandLineArgs(parseCommandLine(this->arguments()))
{
    ...
    setApplicationName("XDown");
    setOrganizationDomain("XDown.org");
#if !defined(DISABLE_GUI)
    setDesktopFileName("org.xdown.download");

#ifdef __WIN_ENABLE_HIGH_DPI__
    setAttribute(Qt::AA_UseHighDpiPixmaps, false);  // opt-in to the high DPI pixmap support
#endif
...
#endif
    const QString appId = QLatin1String("XDown-") + Utils::Misc::getUserIDString() + '@' + instanceId;
    ...
    Logger::instance()->addMessage(tr("XDown %1 started", "XDown v3.2.0alpha started").arg(XDOWN_VERSION));
...
#ifdef __TEST_XDOWN_LOG__
    return settings()->loadValue(KEY_FILELOGGER_ENABLED, true).toBool();
#else
    return settings()->loadValue(KEY_FILELOGGER_ENABLED, false).toBool();
#endif
}
...
void Application::sendNotificationEmail(const BitTorrent::TorrentHandle *torrent)
{
    ...
    smtp->sendMail(pref->getMailNotificationSender(),
                     ...
                     tr("[XDown] '%1' has finished downloading").arg(torrent->name()),
                     content);
}
...
int Application::exec(const QStringList &params)
{
    ...
    try {
        BitTorrent::Session::initInstance(m_commandLineArgs);
        connect(BitTorrent::Session::instance(), &BitTorrent::Session::torrentFinished,
            this, &Application::torrentFinished);
        connect(BitTorrent::Session::instance(), &BitTorrent::Session::allTorrentsFinished,
            this, &Application::allTorrentsFinished, Qt::QueuedConnection);

        connect(BitTorrent::Session::instance(), &BitTorrent::Session::onMainAfter,
            this, &Application::onMainCheckAfter);
    ...
    const QString mesg = QString::fromLatin1("\n******** %1 ********\n").arg(tr("Information"))
        + tr("To control XDown, access the Web UI at %1")
    ...
    bool bIsHideWindow = false;
    
    m_window = new MainWindow(nullptr, bIsHideWindow);

#endif // DISABLE_GUI
    m_running = true;


    BitTorrent::Session::instance()->startUpTorrentTask();
...
void Application::onMainCheckAfter()
{
#ifndef DISABLE_WEBUI
    m_webui = new WebUI;
#ifdef DISABLE_GUI
