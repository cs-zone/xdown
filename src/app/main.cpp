...
#define __USED_QT_STACK_TRACE__
#endif
...
int main(int argc, char *argv[])
{
    ...
    try 
	{
        ...
        if (app->isRunning())
        {
            ...
            if (params.shouldDaemonize)
            {
                throw CommandLineParameterError(QObject::tr("You cannot use %1: XDown is already running for this user.")
        ...
        int iVal = app->exec(params.paramList());
#ifdef __XDOWN_DEBUG__
        _CrtDumpMemoryLeaks();
#endif
        return iVal;
...
bool userAgreesWithLegalNotice()
{
    ...
    const QString eula = QString::fromLatin1("\n*** %1 ***\n").arg(QObject::tr("Legal Notice"))
        + QObject::tr("XDown is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.") + "\n\n"
...
#else
    QMessageBox msgBox;
    msgBox.setText(QObject::tr("XDown is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.\n\nNo further notices will be issued."));
